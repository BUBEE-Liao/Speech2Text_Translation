...
def train(model, optimizer, train_loader):
    ...
    print_iteration = 200
    eval_iteration = 200
    store_iteration = 1000

    for epoch in range(0, epochs):
        print('------epoch{}------'.format(epoch))
        for batch_idx, (audio_padded, audio_feature_padded, audio_feature_mask, src_text_padded, src_text_len, src_text_mask, decoder_input_ids_padded, decoder_input_ids_len, tgt_text_mask, target_ids_padded, target_ids_len) in enumerate(train_loader):
            ...
            speech_encoder_output = s2t_model.forward_speech(audio_feature_padded, audio_feature_mask)
            ...
            speech_decoder_output = s2t_model.text_decoder(input_ids=decoder_input_ids_padded, attention_mask=tgt_text_mask, encoder_hidden_states=speech_encoder_output).last_hidden_state
            ...
            # print(target_long_tensor.get_device())
            # print(text_logits_logSoftmax.get_device())
            # print(speech_logits_logSoftmax.get_device())
            ...
            if(iteration%eval_iteration==0):
                evaluate(s2t_model, audio_padded[0,:], audio_feature_padded[0,:,:], audio_feature_mask[0, :], target_ids_padded[0, :], tokenizer, iteration, writer, device)
            # zero grad
            optimizer.zero_grad()
            # backward
            total_loss.backward()
            ...
            if(iteration%store_iteration==0):
                ckpt_path = os.path.join('/home/bubee/seamless/s2t/ckpt_v2/', "iteration_{}.pth".format(iteration))
...
def evaluate(s2t_model, audio, audio_feature_padded, audio_feature_mask, target_ids_padded, tokenizer, iteration, writer, device):
    s2t_model.eval()
    audio = audio.squeeze()

    audio_feature_padded = audio_feature_padded.unsqueeze(dim=0)
    audio_feature_mask = audio_feature_mask.unsqueeze(dim=0)
    speech_encoder_output = s2t_model.forward_speech(speech_input=audio_feature_padded, attention_mask=audio_feature_mask)
    res, _ = s2t_model.inference(encoder_hidden_state=speech_encoder_output, tokenizer=tokenizer, device=device)
    tgt_text = tokenizer.batch_decode(target_ids_padded, skip_special_tokens=True)
    ...
    writer.add_text('ground_truth_{}'.format(iteration), tgt_text[0], global_step=iteration)
